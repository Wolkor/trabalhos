...
main = do
  let
    ...
    criaStr2 = ["fill:hsl(50," ++ show x ++ "%," ++ show y ++ "%)" | x<-[100,75..0], y<-[100,86..0]]
    criaCoor = [((x,y),56,28) | x<-[10,70..250], y<-[10,40..220]]
    criaCoor2 = [((x,y),56,28) | x<-[340,400..580], y<-[10,40..220]]
    rects = zip criaStr criaCoor
    rects2 = zip criaStr2 criaCoor2
    rect = rects ++ rects2
    (w,h) = (1000,1000)
  writeFile "colors.svg" $ writeRects w h rect